...
def test_sync_source_panels_refreshes_view_before_accessing_objects():
    """
    Ensure that we refresh the view (and thus the session objects) before attempting
    to inspect them. This prevents accessing stale/deleted objects from a previous run.
    """
    # Setup mocks
    mock_app = MagicMock()
    mock_doc = MagicMock()
    ...
    # Stale object simulates an object deleted from the document but still in session list
    class StaleWrapper:
        Name = "StalePanel"
    ...
    # Valid object simulates a fresh object created by show_source_view
    valid_obj = MagicMock()
    ...
    with patch("SquatchCut.core.geometry_sync.App", mock_app), \
        ...
        # When show_source_view is called, we simulate it updating the session to valid objects
        ...
        # Verify the specific warning was NOT logged, confirming we accessed the valid object
