...
class TestSessionStateCoverage:
    ...
    def test_update_job_sheet(self):
        ...
        assert (
            session_state.update_job_sheet(
                0, width_mm=200, height_mm=200, quantity=5, label="Updated"
            )
            is True
        )
    ...
    def test_nesting_stats(self):
        ...
        session_state.set_nesting_stats(
            sheets_used=5, cut_complexity=10.0, overlaps_count=0
        )
        stats = session_state.get_nesting_stats()
...
class TestNestingCoverage:
    ...
    def test_normalize_sheet_definition_invalid(self):
        ...
        assert (
            nesting._normalize_sheet_definition({"width": 100, "height": "invalid"})
            is None
        )

        res = nesting._normalize_sheet_definition(
            {"width": 100, "height": 100, "quantity": "invalid"}
        )
    ...
    def test_derive_sheet_sizes_defaults(self):
        ...
        placements = [
            PlacedPart("p1", 0, 0, 0, 10, 10),
            PlacedPart("p2", 2, 0, 0, 10, 10),
        ]  # uses sheet index 2 -> 3 sheets
        sizes = nesting.derive_sheet_sizes_for_layout(
            "simple", [], 100, 100, placements
        )
        assert len(sizes) == 3
    ...
    def test_compute_utilization_empty(self):
        ...
        assert (
            nesting.compute_utilization_for_sheets([], [])["utilization_percent"] == 0
        )
    ...
    def test_nesting_validation_error(self):
        err = NestingValidationError(
            100, 100, (NestingOffendingPart("p1", 200, 200, False),)
        )
