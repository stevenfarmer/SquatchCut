...
class NestingGroupWidget(QtWidgets.QGroupBox):
    ...
    def _build_ui(self) -> None:
        ...
        self.kerf_width_edit.setToolTip(
            "Blade thickness used to maintain spacing between parts."
        )
        ...
        self.job_allow_rotation_check = QtWidgets.QCheckBox(
            "Allow rotation for this job"
        )
        ...
        # Use vertical layout for buttons to prevent clipping on narrow docks
        button_layout = QtWidgets.QVBoxLayout()
        button_layout.setSpacing(4)

        self.preview_button = QtWidgets.QPushButton("Preview")
        self.preview_button.setToolTip(
            "Preview the nesting layout without leaving the task panel."
        )
    ...
    def apply_state(self, state: dict) -> None:
        ...
        self.cut_mode_check.setChecked(
            nesting_mode == "cut_friendly" or bool(state.get("cut_mode"))
        )

        self.kerf_width_edit.setText(
            sc_units.format_length(
                state.get("kerf_width_mm", 3.0),
                state.get("measurement_system", "metric"),
            )
        )
        self.job_allow_rotation_check.setChecked(
            bool(state.get("job_allow_rotate", False))
        )
    ...
    def _on_settings_changed(self) -> None:
        ...
        session_state.set_nesting_mode(
            "cut_friendly" if self.cut_mode_check.isChecked() else "pack"
        )

        try:
            kw = sc_units.parse_length(
                self.kerf_width_edit.text(), session_state.get_measurement_system()
            )
            if kw > 0:
    ...
    def update_unit_labels(self) -> None:
        ...
        self.kerf_width_edit.setText(
            sc_units.format_length(val, session_state.get_measurement_system())
        )
